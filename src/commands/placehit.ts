...
const PlaceHit: Command = {
    name: "placehit",
    description: "Place a new hit on someone 😈",
    type: "CHAT_INPUT",
    options: [
        {
            name: "target",
    ...
    run: async (client: Client, interaction: BaseCommandInteraction) => {
        ...
        if (price < MINIMUM_HIT_PRICE)
            ...
            // Placer is still under cooldown
                ...
                content = `⏱️ You are still under a cooldown! \`${placer.hiringCooldownString} left!\``;

            // Target is under cooldown
            else if (target.targetingCooldown > 0)
                content = `⏱️ Your target is still under a cooldown from being targeted for a hit. \`${target.targetingCooldownString} left.\``;

            // Target is already under effect of a hit
            else if (isTarget(target))
                content = "⏱️ Your target is currently under the effect of a hit! Wait until 1 hour after that hit is completed.";
